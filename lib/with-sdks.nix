...
let
  ...
  # whose inputs match the usual base-builder fetch so it dedupes at the store
        ...
        value = mkSdk "lib/with-sdks.nix" lang (
          {
            ...
            # build log line and `nix flake check` entry read the same. Matches
            # the flattened `packages.<name>-sdk-<lang>` output the flake
            # module produces. A per-SDK `pname` still wins, since
            # `args.${argName}` merges last.

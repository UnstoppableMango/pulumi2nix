...
# `pythonArgs` is deliberately excluded from that selection: nixpkgs' own
# base provider builder (found via `nixpkgsPath`) already understands
# `pythonArgs` natively and packages the python SDK itself, so it must stay
# in the args forwarded to that base builder untouched - it is not one of
# *this* repo's `lib/sdks`-driven `<lang>Args` blocks, and is not dropped.
{ lib }:
